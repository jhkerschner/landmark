function initMap() {
    var center = {
        ...
        scrollwheel: !1,
        zoom: 15
    }), service = (new google.maps.InfoWindow(), new google.maps.places.PlacesService(map));
    service.getDetails({
...
!function($) {
    ...
    $.fn.bxSlider = function(options) {
        ...
        var windowWidth = $(window).width(), windowHeight = $(window).height(), init = function() {
            ...
            for (var pagerHtml = "", pagerQty = getPagerQty(), i = 0; i < pagerQty; i++) {
                var linkContent = "";
            ...
            "fade" == slider.settings.mode) {
                ...
                value = slider.touch.originalPos.top), !slider.settings.infiniteLoop && (0 == slider.active.index && distance > 0 || slider.active.last && distance < 0) ? setPositionProperty(value, "reset", 200) : Math.abs(distance) >= slider.settings.swipeThreshold ? (distance < 0 ? el.goToNextSlide() : el.goToPrevSlide(), 
        ...
        return el.goToSlide = function(slideIndex, direction) {
            ...
            slider.oldIndex = slider.active.index, slideIndex < 0 ? slider.active.index = getPagerQty() - 1 : slideIndex >= getPagerQty() ? slider.active.index = 0 : slider.active.index = slideIndex, 
    ...
    MainNav.init(), PointerEventsPolyfill.initialize({}), $(".bxslider").bxSlider({
        ...
        pause: 8e3,
        onSliderLoad: function() {
            $(".bx-prev")[0].appendChild($(".slider-control-left svg")[0]), $(".bx-next")[0].appendChild($(".slider-control-right svg")[0]);
        }
    }), $(".page-template-gallery").length && $(".fancybox").fancybox({
        nextEffect: "none",
        prevEffect: "none",
        margin: 50,
        showCloseButton: !1,
    ...
    init: function() {
        $("#menu-top-nav").addClass("visible"), $(".hamburger").on("click touch", function(e) {
            e.preventDefault(), $(".mega-nav").toggleClass("open"), $("#menu-top-nav").toggleClass("visible");
        }), $(".mega-nav-close").on("click touch", function(e) {
            e.preventDefault(), $(".mega-nav").removeClass("open"), $("#menu-top-nav").addClass("visible");
        }), $("#menu-top-nav a").on("click touch", function(e) {
            e.preventDefault();
            var index = $("#menu-top-nav a").index(this);
            console.log(index), $("#menu-mega-nav > li").removeClass("focused"), $($("#menu-mega-nav > li").get(index)).addClass("focused"), 
            $(".mega-nav").addClass("open"), $("#menu-top-nav").removeClass("visible");
        });
    }
}, $("a").click(function() {
    category = $(this).attr("data-event-category"), "undefined" != typeof category && category !== !1 && (action = $(this).attr("data-event-action"), 
    label = $(this).attr("data-event-label"), ga("send", "event", category, action, label));
});